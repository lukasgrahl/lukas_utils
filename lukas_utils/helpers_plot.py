...
def _is_df_equal(lst_dfs, is_raise: bool = True):
    if len(lst_dfs) == 1:
        pass
    else:
        lst0 = [
...
def _plot_func_basis(
    ...
    _is_df_equal(list(dict_dfs.values()), is_raise=is_raise)
...
def get_ndf_plot(
    ...
    is_raise_equivalence: bool = True,
):
    dict_dfs, is_legend, _ = _plot_func_basis(
        dfs,
        is_raise=is_raise_equivalence,
    )
